# Bluetooth Low Energy開発実践 - 章立て

**書籍タイトル**: Bluetooth Low Energy開発実践  
**想定ページ数**: 300-400ページ  
**最終更新**: 2025-10-23  
**ステータス**: Draft v1.0

---

## 書籍概要

### 対象読者

- 組み込みエンジニア（ファームウェア開発者）
- モバイルアプリ開発者（iOS/Android）
- プロダクトマネージャー・企画立案者
- ホビイスト（上記役割を兼ねる個人開発者）

### 本書の目的

BLE開発における各担当者（ファームウェア/アプリ/企画）がBLE知識をもとに会話し、技術的連携を生み出すための共通言語を提供します。SDKで隠蔽された通信の実態を理解し、チーム全体で深い理解に基づくデバッグと問題解決ができるレベルを目指します。

### 読者が得られる成果

- BLEファームウェアとアプリケーションを開発し連携させられる（試作レベル）
- SDKの操作が実際の通信プロトコルでどう実現されているか説明できる
- パケットキャプチャを読み解き、ハード/ファーム/アプリ/OSの問題を切り分けられる
- BLEで実現可能かどうか判断し、機能設計と開発者とのコミュニケーションができる

### 技術スタック

- **ファームウェア**: Nordic nRF52840 + nRF Connect SDK 3.0
- **アプリケーション**: iOS Core Bluetooth（メイン）、Web Bluetooth API、Android概要、Linux概要

### GitHub公開

公開GitHubリポジトリには、本文全体ではなくサポート資料を置く予定：
- リポジトリ: https://github.com/reinforce-lab/ble-in-action
- 公開対象: サンプル章、正誤表、サンプルコード、ビルド環境、図表の一部
- ビルドシステム: Pandoc + Makefile（EPUB/PDF生成）
- イシュー受付: 誤字脱字・技術的指摘をGitHubイシューで受付

### 前提知識

高校生レベルの基礎知識 + micro:bit等のSTEM教材を体験済みを推奨。ゼロから始める読者には序文で適切な教材を紹介します。

### 執筆ガイドライン

各章の執筆では、以下の文書を参照します。

- **docs/book-constitution.md**: 本書の執筆理念、品質基準、コード例の検証方針
- **docs/writing-style-guide.md**: 表記、用語、Markdown/Pandoc書式
- **docs/writing-guidelines-reader-notes.md**: 読者層別Noteの挿入方針、章別配慮マップ、記述例

各章では、必要に応じて【ホビーストの方へ】【ファーム開発者の方へ】【アプリ開発者の方へ】【企画者の方へ】のNoteを挿入します。

---

## 序文

### 0.1 本書の目的
**概要**: なぜこの本を書いたか、BLE開発における「共通言語」の重要性  
**想定ページ数**: 1-2ページ  
**前提章**: なし

### 0.2 想定読者と各層へのメリット
**概要**: 4つの読者層（ホビースト、ファーム開発者、アプリ開発者、企画者）の特定と、各層が本書から得られる具体的なメリット  
**想定ページ数**: 2-3ページ  
**前提章**: なし

#### 0.2.1 ホビーストの方へ
- **あなたの強み**: 一人で全てを担える自由度、低コストでの実現可能性
- **スタート地点**: STEM教材（micro:bit、M5Stack、Adafruit Feather nRF52840等）の改造から
- **本書で得られるもの**: ファーム（第5-8章）とアプリ（第9-11章）の基礎、実践プロジェクト（第12-15章）
- **学習の進め方**: 第1-2章で基礎を理解 → 第5章でファーム環境構築 → 第9章でiOSアプリ基礎 → 第12-15章で統合プロジェクト

#### 0.2.2 ファーム開発者の方へ
- **あなたの強み**: 組み込み、センサー、消費電力、リアルタイム制御の知識
- **共通の課題**: アプリ側とのGATT設計、UUID、接続パラメータ、エラー処理のすり合わせ
- **本書で得られるもの**: アプリ側の制約、BLE仕様とSDK実装の対応、デバッグスキル（第15-16章）
- **学習の進め方**: 第1-4章で共通基盤理解 → 第5-8章を深く学習 → 第9-11章でアプリ側を概要把握 → 第15-16章でデバッグ手法習得
- **すり合わせポイント**: GATT構造の事前設計、UUID命名規則、接続パラメータ調整、通知頻度、エラーハンドリングの共通理解

#### 0.2.3 アプリ開発者の方へ
- **あなたの強み**: OS、UI、アプリ状態管理、ユーザー権限、バックグラウンド動作の知識
- **共通の課題**: ファーム側で定義されたGATTと、OS APIの制約を接続すること
- **本書で得られるもの**: ファーム側のGATT設計意図、Core BluetoothなどのAPIが隠蔽する通信の理解、デバッグスキル（第15-16章）
- **学習の進め方**: 第1-4章で共通基盤理解 → 第9-11章を深く学習 → 第5-8章でファーム側を概要把握 → 第15-16章でデバッグ手法習得
- **すり合わせポイント**: UUID、CharacteristicのRead/Write/Notify、権限、スキャン制約、バックグラウンド動作、OS差分

#### 0.2.4 企画者の方へ
- **あなたの強み**: 市場ニーズの把握、ユーザー視点でのアイディア創出
- **BLEの可能性**: スマホでは取れない情報、感覚器官への訴求、生体情報の活用
- **本書で得られるもの**: BLEの技術的可能性と制約の理解、実現可能性の判断力、開発者との対話力
- **学習の進め方**: 第1-2章で基礎概念 → 第12-15章で実例から発想 → 必要に応じて技術詳細章を参照
- **アイディア発想の軸**:
  - 生体情報軸: 心拍、体温、動き、汗、脳波
  - 装着部位軸: 手首、耳、胸、腰、足、衣服
  - センサー軸: 加速度、ジャイロ、温度、湿度、圧力、光
  - 用途軸: 健康管理、スポーツ、エンタメ、業務効率化、安全管理

### 0.3 本書の活用方法
**概要**: 読者層別の推奨読書順序、各Part・各章の位置づけ、リファレンス的な使い方  
**想定ページ数**: 2-3ページ  
**前提章**: なし

#### 読者層別 推奨読書順序
- **ホビースト**: 序文 → Part 1（全員必須）→ Part 3（第5-6章）→ Part 4（第9章）→ Part 5（プロジェクト実践）
- **ファーム開発者**: 序文 → Part 1-2（基礎と通信規格）→ Part 3（詳細）→ Part 4（第9章概要）→ Part 6（デバッグ）
- **アプリ開発者**: 序文 → Part 1-2（基礎と通信規格）→ Part 4（詳細）→ Part 3（第6章概要）→ Part 6（デバッグ）
- **企画者**: 序文 → Part 1（基礎知識）→ Part 5（実例）→ 必要に応じて技術詳細章を参照

### 0.4 サポートリポジトリとビルド方法
**概要**: サンプル章、正誤表、サンプルコード、ビルド環境、図表の一部の公開場所、Pandoc+Makefileでのビルド手順、イシュー報告方法  
**想定ページ数**: 2-3ページ  
**前提章**: なし

### 0.5 前提知識と推奨教材
**概要**: micro:bit等のSTEM教材紹介、電子工学の基礎、プログラミング言語の選択  
**想定ページ数**: 2-3ページ  
**前提章**: なし

**序文合計**: 9-14ページ

---

## Part 1: BLE基礎知識（全員必須）

**Part目的**: BLEとは何か、なぜ選ばれるのか、基本的な概念と用語を理解する。ファーム/アプリ/企画全ての読者が共通して理解すべき基盤知識。

### 第1章: BLEとは何か
**概要**: BLEの歴史、Bluetooth Classicとの違い、主な用途、低消費電力の仕組み、なぜBLEが選ばれるのか  
**想定ページ数**: 18〜23ページ  
**前提章**: 序文  
**ファイル**: `manuscript/chapters/01-what-is-ble/`  
**詳細**: [specs/004-chapter-01-what-is-ble/spec.md](004-chapter-01-what-is-ble/spec.md) を参照

### 第2章: BLEの基本概念と用語
**概要**: セントラル/ペリフェラル、接続と広告、UUID、サービスとキャラクタリスティック、通信フロー概要  
**想定ページ数**: 21〜28ページ  
**前提章**: 第1章  
**ファイル**: `manuscript/chapters/02-ble-basic/`  
**詳細**: [manuscript/chapters/02-ble-basic/outline.md](../manuscript/chapters/02-ble-basic/outline.md) を参照

**Part 1合計**: 40-50ページ

---

## Part 2: BLE通信規格の詳細（レイヤー別）

**Part目的**: SDKの裏側で何が起きているかを理解する。パケットレベルでの通信を把握し、デバッグの基盤とする。

### 第3章: 物理層・リンク層・GAP
**概要**: 2.4GHz帯の物理層、周波数ホッピング、広告パケット構造、スキャンと接続確立、GAPの役割  
**想定ページ数**: 25-30ページ  
**前提章**: 第2章  
**ファイル**: `manuscript/chapters/03-ble-layer/`  
**詳細**: [manuscript/chapters/03-ble-layer/outline.md](../manuscript/chapters/03-ble-layer/outline.md) を参照

### 第4章: ATT・GATT・プロファイル
**概要**: ATTプロトコル、GATTデータ構造、サービスとキャラクタリスティックの詳細、Read/Write/Notify/Indicate、標準プロファイル  
**想定ページ数**: 28-33ページ  
**前提章**: 第3章  
**ファイル**: `manuscript/chapters/04-att-gatt/`  
**詳細**: [manuscript/chapters/04-att-gatt/outline.md](../manuscript/chapters/04-att-gatt/outline.md) を参照

**Part 2合計**: 53-63ページ

---

## Part 3: ファームウェア開発編（nRF52840）

**Part目的**: Nordic nRF52840でBLEペリフェラルデバイスを実装する。低消費電力設計と外部チップ連携の実践知識を習得。

### 第5章: nRF52840開発環境
**概要**: nRF Connect SDK 3.0のセットアップ、開発ツール（VS Code, nRF Connect for Desktop）、デバッガ接続、Hello World  
**想定ページ数**: 20-24ページ  
**前提章**: 第2章  
**ファイル**: `manuscript/chapters/05-nrf52840-setup/`  
**詳細**: [manuscript/chapters/05-nrf52840-setup/outline.md](../manuscript/chapters/05-nrf52840-setup/outline.md) を参照

### 第6章: ペリフェラル実装
**概要**: 広告の開始、カスタムサービスとキャラクタリスティックの定義、Read/Write/Notifyの実装、接続管理  
**想定ページ数**: 30-35ページ  
**前提章**: 第4章、第5章  
**ファイル**: `manuscript/chapters/06-peripheral-impl/`  
**詳細**: [manuscript/chapters/06-peripheral-impl/outline.md](../manuscript/chapters/06-peripheral-impl/outline.md) を参照

### 第7章: 低消費電力設計
**概要**: スリープモード、広告間隔の最適化、接続間隔の調整、電流測定とプロファイリング、バッテリー駆動の実践  
**想定ページ数**: 25-30ページ  
**前提章**: 第6章  
**ファイル**: `manuscript/chapters/07-low-power-design/`  
**詳細**: [manuscript/chapters/07-low-power-design/outline.md](../manuscript/chapters/07-low-power-design/outline.md) を参照

### 第8章: 外部チップ連携（4線IF・LTE統合）
**概要**: UART/SPI接続、LTEチップ+nRF52840構成（2パターン）、OTA実装の考慮点、トラブルシューティング  
**想定ページ数**: 28-33ページ  
**前提章**: 第6章、第7章  
**ファイル**: `manuscript/chapters/08-external-chips/`  
**詳細**: [manuscript/chapters/08-external-chips/outline.md](../manuscript/chapters/08-external-chips/outline.md) を参照

**Part 3合計**: 103-122ページ

---

## Part 4: アプリケーション開発編

**Part目的**: iOS、Web、Android（概要）、Linux（概要）でBLEセントラルアプリを実装する。各プラットフォームのAPI特性を理解。

### 第9章: iOS Core Bluetooth
**概要**: Core Bluetoothフレームワーク、スキャン、接続、サービス・キャラクタリスティックのディスカバリ、Read/Write/Subscribe、エラーハンドリング  
**想定ページ数**: 35-40ページ  
**前提章**: 第4章  
**ファイル**: `manuscript/chapters/09-ios-core-bluetooth/`  
**詳細**: [manuscript/chapters/09-ios-core-bluetooth/outline.md](../manuscript/chapters/09-ios-core-bluetooth/outline.md) を参照

### 第10章: Web Bluetooth API
**概要**: Web Bluetooth APIの概要、ブラウザ対応状況、デバイス選択UI、Promiseベースの非同期処理、Webアプリでのセキュリティ考慮  
**想定ページ数**: 22-27ページ  
**前提章**: 第4章  
**ファイル**: `manuscript/chapters/10-web-bluetooth/`  
**詳細**: [manuscript/chapters/10-web-bluetooth/outline.md](../manuscript/chapters/10-web-bluetooth/outline.md) を参照

### 第11章: Android概要とLinux概要
**概要**: Android BLE APIの構造（サンプルコード）、BlueZ on Linux（概要のみ）、各プラットフォームの特徴比較  
**想定ページ数**: 18-22ページ  
**前提章**: 第4章  
**ファイル**: `manuscript/chapters/11-android-linux-ble/`  
**詳細**: [manuscript/chapters/11-android-linux-ble/outline.md](../manuscript/chapters/11-android-linux-ble/outline.md) を参照

**Part 4合計**: 75-89ページ

---

## Part 5: プロジェクト実践（統合）

**Part目的**: Part 3（ファーム）とPart 4（アプリ）の知識を統合し、実際の製品を作る。ビーコン、温度計、OTA（DFU）の3プロジェクト。

### 第12章: ビーコン実装
**概要**: iBeacon/Eddystoneの仕様、ペリフェラル側実装、iOSアプリでの受信、位置情報サービスとの連携  
**想定ページ数**: 20-24ページ  
**前提章**: 第6章、第9章  
**ファイル**: `manuscript/chapters/12-beacon-impl/`  
**詳細**: [manuscript/chapters/12-beacon-impl/outline.md](../manuscript/chapters/12-beacon-impl/outline.md) を参照

### 第13章: 温度計実装
**概要**: センサー（DHT22等）との接続、温度データのBLE送信、iOSアプリでのグラフ表示、Notificationの活用  
**想定ページ数**: 22-27ページ  
**前提章**: 第6章、第9章  
**ファイル**: `manuscript/chapters/13-thermometer-impl/`  
**詳細**: [manuscript/chapters/13-thermometer-impl/outline.md](../manuscript/chapters/13-thermometer-impl/outline.md) を参照

### 第14章: OTA（DFU）実装
**概要**: **重点解説** - Nordic DFUプロトコル、Bootloaderの仕組み、ファームウェア更新フロー、iOSアプリからのDFU実行、よくあるトラブルと解決法  
**想定ページ数**: 30-36ページ  
**前提章**: 第6章、第8章、第9章  
**ファイル**: `manuscript/chapters/14-ota-dfu/`  
**詳細**: [manuscript/chapters/14-ota-dfu/outline.md](../manuscript/chapters/14-ota-dfu/outline.md) を参照

**Part 5合計**: 72-87ページ

---

## Part 6: デバッグとトラブルシューティング

**Part目的**: 問題発生時に、パケットキャプチャで原因を特定し解決する力を養う。よくある問題のパターンを学ぶ。

### 第15章: パケットキャプチャとデバッグ
**概要**: nRF Snifferの使い方、Wiresharkでのパケット解析、接続失敗のデバッグ、データ送信失敗のデバッグ、実例解説  
**想定ページ数**: 25-30ページ  
**前提章**: 第3章、第4章  
**ファイル**: `manuscript/chapters/15-debugging/`  
**詳細**: [manuscript/chapters/15-debugging/outline.md](../manuscript/chapters/15-debugging/outline.md) を参照

### 第16章: よくある問題と解決法
**概要**: 接続できない、データが送れない、電池が持たない、OTAが失敗する、プラットフォーム固有の問題、FAQ  
**想定ページ数**: 22-27ページ  
**前提章**: 第15章  
**ファイル**: `manuscript/chapters/16-troubleshooting/`  
**詳細**: [manuscript/chapters/16-troubleshooting/outline.md](../manuscript/chapters/16-troubleshooting/outline.md) を参照

**Part 6合計**: 47-57ページ

---

## 付録

### 付録A: BLE仕様書の読み方
**概要**: Bluetooth SIG仕様書の構成、必要な部分の見つけ方、仕様書とSDKドキュメントの使い分け  
**想定ページ数**: 8-10ページ  
**前提章**: 第3章、第4章  
**ファイル**: `manuscript/back-matter/appendix-a-ble-spec.md`

### 付録B: 参考文献
**概要**: 公式仕様書、技術書、Webリソース、SDKドキュメントのリンク集  
**想定ページ数**: 4-6ページ  
**前提章**: なし  
**ファイル**: `manuscript/back-matter/appendix-b-references.md`

**付録合計**: 12-16ページ

---

## 全体サマリー

| Part | 章数 | 想定ページ数 | 主な内容 |
|------|------|--------------|----------|
| 序文 | 4節 | 10-14p | 本書の目的、使い方、GitHub、前提知識 |
| Part 1: 基礎知識 | 2章 | 40-50p | BLEとは（1.1〜1.5節）、基本概念と用語 |
| Part 2: 通信規格 | 2章 | 53-63p | 物理層〜GAP、ATT〜GATT |
| Part 3: ファームウェア | 4章 | 103-122p | nRF52840、ペリフェラル実装、低消費電力、外部チップ連携 |
| Part 4: アプリケーション | 3章 | 75-89p | iOS、Web、Android概要、Linux概要 |
| Part 5: プロジェクト実践 | 3章 | 72-87p | ビーコン、温度計、OTA（DFU） |
| Part 6: デバッグ | 2章 | 47-57p | パケットキャプチャ、よくある問題 |
| 付録 | 2 | 12-16p | 仕様書の読み方、参考文献 |
| **合計** | **16章+付録** | **410-495p** | - |

**注**: 当初目標は300-400ページだが、内容の充実を優先し400-500ページ程度を許容範囲とする。

---

## 章の依存関係

```
序文
 ├─ 第1章（BLEとは）
 │   └─ 第2章（基本概念）
 │       ├─ 第3章（物理層〜GAP）
 │       │   └─ 第4章（ATT〜GATT）
 │       │       ├─ 第5章（nRF52840環境）
 │       │       │   └─ 第6章（ペリフェラル実装）
 │       │       │       ├─ 第7章（低消費電力）
 │       │       │       ├─ 第8章（外部チップ連携）
 │       │       │       └─ 第12章（ビーコン実装）※第9章も必要
 │       │       │           └─ 第13章（温度計実装）※第9章も必要
 │       │       │               └─ 第14章（OTA実装）※第8章、第9章も必要
 │       │       ├─ 第9章（iOS Core Bluetooth）
 │       │       ├─ 第10章（Web Bluetooth）
 │       │       └─ 第11章（Android/Linux概要）
 │       └─ 第15章（パケットキャプチャ）
 │           └─ 第16章（よくある問題）
 └─ 付録A（仕様書の読み方）※第3章、第4章前提
     └─ 付録B（参考文献）
```

---

## 変更履歴

### 2026-02-22: v1.3 第2章サブ節アウトラインを chapter outline に移動
- 第2章の詳細（2.1〜2.7節）を `manuscript/chapters/02-ble-basic/outline.md` に新規作成
- `specs/outline.md` の第2章エントリを概要+参照リンクに簡略化

### 2026-02-22: v1.2 第1章サブ節アウトラインを chapter spec に移動
- 第1章のサブ節詳細（1.1〜1.5）を `specs/004-chapter-01-what-is-ble/spec.md` に移動
- `specs/outline.md` の第1章エントリを概要のみに簡略化

### 2026-02-22: v1.1 第1章サブ節構成を追加
- 第1章を5節（1.1〜1.5）に分割、各節のタイトル・概要・ページ数を定義
- `manuscript/chapters/01-what-is-ble/` のファイルリスト（1.1〜1.5）に対応づけ
- 第1章合計を18〜23ページに更新、Part 1合計を40〜50ページに修正

### 2025-10-23: v1.0 初版
- 全16章+付録の章立て作成
- 各章の概要、想定ページ数、依存関係を定義
- 技術スタック（nRF52840, iOS, Web, Android概要）を明記
- プロジェクト実践編（ビーコン、温度計、OTA）を配置
- 総ページ数410-495p（内容充実を優先、400-500p許容）

---

## 次のステップ

1. **各章の詳細仕様作成**: `specs/` の既存仕様を参照しながら、章ごとに進める
2. **並行執筆**: 依存関係のない章（第1章、第5章、第9章、第10章、第11章）から開始可能
3. **レビュー**: 技術レビュアーによる章立て全体の評価
