...
micro:bit は、英国放送協会(BBC, British Broadcasting Corporation)が主体となり開発された、マイクロ・コンピュータやセンサーを搭載している教育むけマイコン・ボードです。このデバイスや教育教材は、英国に本部を置くMicro:bit教育財団という非営利団体が運用しています。

このmicro:bitとスマートホンとの無線通信に、Bluetooth Low Energy技術が使われています。Bluetooth Low Energy技術を教育教材で始めるのは、利点がたくさんあります。


使っている人数が多い、それがスタートして動かしている。体験のスタートして、よい。
開発環境の立ち上げ、動かすまで大変。
入手性、お値段、販売しているところ。電子部品。

逆に、プロフェッショナル向けだと、その辺りが、プロフェッショナルならできますよね、になっているので、初めての場合だと、ちょっと苦労するかも?
...
開発としてはプロ向けそのもの。nRF52840、豊富なセンサー、加速度、タッチセンサー、LED表示、汎用入出力端子。プログラミング環境自体は、使いやすく。しかし、ベースになっているものは、商業開発で用いる、そのもの。実際これで、商業の開発の試作に使うものと、ハードウェア構成として、何も違いがない。



スマートホンからロボットを動かしたり。
教材、豊富な。まず動かせること。利用者が多いので、参考にして、コピペでできる教材が多い。わからないときの相談が共有できる。


<!-- 構成: iPhoneと電子回路。物だけじゃなくて、ソフトの開発環境。スマホ側とマイクロコントローラ側-->
<!-- メリット: 公式の教材が豊富。体験の共有基盤。ハードは同じものがあるので、コピペでトレースできる。わからないことがあっても、聞ける。使っている人が多いので、踏み抜くトラブルとその解決とかネットのやり取りで何かあり、検索すれば出てきやすい。>
<!-- 子供向けではない。内容物としては、プロが使うのと同じ。発展として、構成も見ていきましょう。-->

#### 初めてのBluetooth Low Energy技術

iPhoneとmicro:bitを通信で接続して動かしてみます。クイック・スタートが、[iOSでmicro:bitを使用する - Bluetooth経由の接続](https://microbit.org/ja/guide/ble-ios/)にまとめられています。
...
まず、micro:bitを入手します。[BBC micro:bitとBBC micro:bitアクセサリはどこで購入できるか](https://microbit.org/ja/resellers/)に購入先リストがあります。micro:bit本体は、2000円ほどで販売されています。

次に、App Storeから、[micro:bitのiOSアプリケーション](https://apps.apple.com/gb/app/micro-bit/id1092687276)を入手します。
...
ここまで動かせれば、あとは、新たな組み合わせに挑戦したり、遊び方を工夫したり、自分の思うままに遊びます。何をすればわからないならば、アプリケーション下部にある"Idea"ボタンを押すと、
<https://microbit.org/guide/ble-ios/#activity>に、また日本語では [レッスン、プロジェクト、他のアイデア](https://microbit.org/ja/ideas/)に、micro:bitを使う様々なアイディアとその作り方が多数掲載されています。また書籍もあります。
...
ここで見ている構成。
人から見ると、スマホから何かを操作している。あるいはデバイスのスイッチとか状態を、スマホで見ている。
スマホのBLE機能。スマホのアプリ。BLEの機能(無線+スタック)。通信。BLEの機能(無線+スタック)。マイコンのファーム。許される部分は、アプリのAPI、マイコンのすべて。周辺回路。通信部分は使うだけ。

ここで見ていない項目、バックグラウンド。勝手に。人が見ていない、人が操作しない時の何か。 -->


### 通信のやり取りの中身を見てみる

<!-- 発見と接続、GATT基盤、サービスとキャラクタリスティクス。
スマホのアプリから見えるAPIを、手動で動かす。 -->
...
Bluetooth Low Energy技術の通信を見るアプリケーションは、いくつかありますが、ここでは、 [nRF Connect](https://apps.apple.com/jp/app/nrf-connect/id1054362403)を使います。
...
| 駅や空港  |  新規製品(落し物検出、スマートロック、カーテン開け閉め) |   健康、安全、見守り100億円の売り上げ、
