...
- [JTF日本語標準スタイルガイド（翻訳用）第3.0版](https://www.jtf.jp/tips/styleguide) (2019, CC BY 4.0) - カタカナ複合語・長音表記の基準
- [JTCA 外来語（カタカナ）表記ガイドライン 第3版](https://jtca.org/tcwp/wp-content/uploads/2023/06/katakana_guide_3_20171222.pdf) (2015) - 語尾の長音表記の基準
...
### 3.4 カタカナ複合語の区切り（中黒の扱い）

**原則**: カタカナ複合語には中黒（・）を**使用しない**。複合語を1語として続けて書きます。

**背景**: JTF日本語標準スタイルガイド（2019）の規則6（2.1.7項）は、長いカタカナ複合語に中黒または半角スペースによる区切りを推奨しています。一方、BLE関連の技術文書やNordic Semiconductor社のドキュメントでは、中黒なしで1語として書く慣行が定着しています。本書では、ターゲット読者（開発者・エンジニア）が慣れ親しんでいる業界慣行に合わせ、中黒なしで統一します。
...
**参考規格**:
- JIS Z 8301（規格票の様式及び作り方）: 語尾の長音符号の規則のみを定めており、中黒による複合語区切りについての規定はない
- JTF日本語標準スタイルガイド 2.1.7項: 長い複合語には中黒を推奨するが、分野の慣行に従ってよいとしている
- JTCA外来語表記ガイドライン第3版: 語尾の長音表記が主な対象で、複合語の区切りについての規定はない

### 3.5 語尾の長音表記

**原則**: 3音以上のカタカナ語の語尾長音は**省略しない**（長音符号「ー」を付ける）。
...
**例外**: 業界で定着している短縮形はそのまま使用します。
```
センサー（センサも許容）
```

**背景**: JIS Z 8301は3音以上の語尾長音を省略してよいとしていますが、文化庁「公用文作成の考え方」(2021)や一般的な技術書の慣行では長音を付ける表記が主流になっています。本書は読みやすさを重視し、長音を付ける方を採用します。
...
**図番号の体系**: `Fig. {章番号}.{連番}` を使用します。Pandocの出力やPDF/EPUBでの参照に統一性を持たせるため、本文中では `Fig. 1.1` のように英語表記で参照します。

### 5.1.1 サブ図（複数パネルの図）

操作手順のスクリーンショットなど、関連する複数の画像を1つの図としてまとめる場合は、`(a)`, `(b)`, `(c)` のサブ図ラベルを使用します。

```markdown
![nRF Connectの操作画面（プレースホルダー）](images/screenshots/nrf-connect-example.png)

**Fig. 1.1**: nRF Connectでmicro:bitに接続した画面。
(a) Scanner画面——周囲のBLEデバイスを一覧表示する。
(b) Service一覧画面——接続後に表示されるサービスとキャラクタリスティック。
(c) Accelerometer Dataの値表示——micro:bitを傾けると値が変化する。
(d) LED Textへの書き込み——文字列を入力しWriteを押す。
```

**本文中の参照**: `Fig. 1.1(a)に示すように…` のように、サブ図ラベルを括弧付きで記述します。

**ルール**:
- サブ図ラベルは小文字のアルファベット `(a)`, `(b)`, … を使用
- 各サブ図にはキャプション（短い説明文）を付ける
- サブ図間は改行で区切る
- IEEE/学術論文の慣行に準拠
...
**改訂履歴**:
- 1.1.0 (2026-02-25): カタカナ複合語の中黒ルール（3.4節）、語尾長音表記（3.5節）、サブ図の表記規則（5.1.1節）を追加。JTF/JTCAガイドライン参照を追加
